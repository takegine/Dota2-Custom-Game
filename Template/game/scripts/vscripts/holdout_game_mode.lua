...
local HOLDOUT_CONFIG_FILE = "scripts/maps/zombies.txt"	--"scripts/maps/" .. GetMapName() .. ".txt"
HOLDOUT_CONFIG_FILE = "scripts/maps/holdout.txt"
local HOLDOUT_TARGET_ANCIENT	=	false
...
-- Precache resources
function PrecacheCHoldoutGameMode( context )
...
function ActivateCHoldoutGameMode()
...
function CHoldoutGameMode:InitGameMode()
	...
	self.configFile = self.configFile or HOLDOUT_CONFIG_FILE
	self:_ReadGameConfiguration(self.configFile)
	--[[
...
function CHoldoutGameMode:_ReadGameConfiguration(configFile)
	local kv = LoadKeyValues( configFile )
...
function CHoldoutGameMode:_AllPlayersDead()
...
function CHoldoutGameMode:_CheckForDefeat()
	if GameRules:State_Get() == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
		if self:_AllPlayersDead() and not self._ischeckingdefeat then
			GameRules:GetGameModeEntity():SetThink("CheckForDefeatDelay", self, 0.5)
			self._ischeckingdefeat = true
		end

		if HOLDOUT_TARGET_ANCIENT and self._entAncient then
			if not self._entAncient:IsAlive() then
				GameRules:MakeTeamLose( DOTA_TEAM_GOODGUYS )	--	GameRules:SetGameWinner(DOTA_TEAM_BADGUYS)
			end
		end
	end
end

function CHoldoutGameMode:CheckForDefeatDelay()
	if self._defeatcounter > 0 then
		...
		if self:_AllPlayersDead() then
			if HOLDOUT_TARGET_ANCIENT and self._entAncient and self._entAncient:IsAlive() then
